...
class _LayoutOverflowError(Exception):
    """Raised when a page layout overflows, indicating pagination data is stale."""
    pass
...
class PDFPreviewWidget(QWidget):
    ...
    def _layout_all_pages(self):
        ...
        # If spacing changed or units mismatch, we may detect an overflow during rendering.
        # In that case, trigger a full reflow (repagination) and retry once.
        for attempt in range(2):
            try:
                ...
                return  # success
            except _LayoutOverflowError as oe:
                print(f"DEBUG: Layout overflow detected in preview -> forcing full reflow: {oe}")
                if attempt == 1:
                    raise
                # Find dialog (walk up parents) and call the same pagination logic used elsewhere
                dlg = self.parent()
                while dlg is not None and not hasattr(dlg, '_organize_into_pages_pdf_export_logic'):
                    dlg = dlg.parent()
                if dlg is None:
                    # No paginator available; keep old pages
                    return
                # Reset stale per-question metadata and repaginate
                for q in self.questions:
                    q.page_num = 0
                    q.col_num = 0
                self.pages = dlg._organize_into_pages_pdf_export_logic(self.questions)
                continue
        ...
        try:
            ...
            for idx, q in enumerate(page_questions):
                ...
                if (y_px + total_needed_px) > max_y:
                    # Pagination data is stale (e.g., spacing changed) or unit conversion mismatch.
                    # Do NOT render overflowing content; trigger a full reflow.
                    raise _LayoutOverflowError(
                        f"Preview layout overflow on page {page_idx+1}: q#{getattr(q.selection,'number', '?')} y={y_px} needed={total_needed_px} max_y={max_y} col={col_idx}"
                    )
